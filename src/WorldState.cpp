...
WorldState::WorldState(const std::string& path) {
    ...
    for (const auto& [fact_name, fact_value] : data.items()) {
        if (fact_value.is_boolean()) {
            ...
            bool all_bool = std::all_of(fact_value.begin(), fact_value.end(), [](const json& v){ return v.is_boolean(); });
            bool all_int  = std::all_of(fact_value.begin(), fact_value.end(), [](const json& v){ return v.is_number_integer(); });
            bool all_double = std::all_of(fact_value.begin(), fact_value.end(), [](const json& v){ return v.is_number_float(); });
            bool all_string = std::all_of(fact_value.begin(), fact_value.end(), [](const json& v){ return v.is_string(); });
