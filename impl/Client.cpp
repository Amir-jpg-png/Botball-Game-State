...
std::unique_ptr<GameState> Client::get_remote_state(const std::string &ip_address, const uint16_t port) {
...
std::unique_ptr<GameState> Client::parse_remote_state(json data) const {
    json table_data;
    try {
    ...
    phase_state = std::make_unique<PhaseState>();
