...
void Socket::recv_all(void *buf, const size_t len) {
    size_t total = 0;
...
void Socket::send_all(const void *buf, const size_t len) const {
    size_t total = 0;
    while (total < len && !m_closed) {
        const ssize_t n = send(m_fd, static_cast<const char *>(buf) + total, len - total, MSG_NOSIGNAL);
        if (n < 0) fatal("error: connection lost", m_log);
        total += n;
    }
}
...
std::optional<json> Socket::recv_json() {
    ...
    constexpr uint32_t MAX_MSG = 1 << 20; // 1MB safety
