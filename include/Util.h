...
inline std::map<std::string, std::any> get_key_value(const json &data) {
    ...
    for (const auto &[condition_key, condition_value]: data.items()) {
        if (condition_value.is_boolean()) {
            ...
            const bool all_string = std::ranges::all_of(condition_value.begin(), condition_value.end(),
...
inline std::unordered_map<std::string, std::any> get_key_value_unordered_map(const json &data) {
